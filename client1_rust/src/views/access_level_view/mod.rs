...
use web_sys::{
    Document, HtmlButtonElement, HtmlElement, HtmlInputElement, HtmlTextAreaElement, // Added HtmlTextAreaElement
...
const API_URL: &str = "/securityAccessLevel";

#[derive(Serialize)]
struct AccessLevelCreate {
    name: String,
    description: String,
}

#[derive(Serialize)]
struct AccessLevelUpdate {
    name: String,
    description: String,
}
...
impl AccessLevelView {
    pub fn new(
        ...
        state_div.set_id("alv_state_div");
        ...
        AccessLevelView::fetch_items(view_rc.clone());

        console_log!("AccessLevelView: Instance created and fetch_items called.");
        Ok(view_rc)
    }

    pub fn fetch_items(view_rc: Rc<RefCell<Self>>) {
        console_log!("AccessLevelView: Fetching items...");
        ...
        spawn_local(async move {
            console_log!("AccessLevelView: Requesting data from {}", API_URL);
            match http_client.get(API_URL).send().await {
                Ok(response) => {
                    if response.status().is_success() {
                        console_log!("AccessLevelView: Received successful response.");
                        match response.json::<Vec<TableData>>().await {
                            Ok(fetched_records) => {
                                console_log!("AccessLevelView: Parsed {} records.", fetched_records.len());
                                *records_rc_internal.borrow_mut() = fetched_records;
                                
                                if let Err(e) = AccessLevelView::render_records(
                                    &document_clone,
                                    &list_div_clone,
                                    &records_rc_internal.borrow(),
                                    view_rc_clone_for_async.clone(),
                                ) {
                                    console_log!("AccessLevelView: Error rendering records: {:?}", e);
                                }
                            }
                            Err(e) => {
                                console_log!("AccessLevelView: Error parsing JSON: {:?}", e);
                            }
                        }
                    } else {
                        console_log!("AccessLevelView: Request failed with status: {}", response.status());
                    }
                }
                Err(e) => {
                    console_log!("AccessLevelView: Network error: {:?}", e);
                }
            }
        });
    }
    ...
    pub fn render_records(
        ...
        if records.is_empty() {
            ...
            no_items_msg.set_inner_text("No access levels found.");
        ...
        for record in records.iter() {
            ...
            text_span.set_inner_text(&format!(
                "{} (ID: {}) - Desc: {} - Created: {}, Modified: {}", // Display name and description
            ...
            let edit_closure = Closure::wrap(Box::new(move |_event: web_sys::MouseEvent| {
                if let Err(e) = AccessLevelView::populate_edit_form(view_rc_clone_for_edit.clone(), item_clone_for_edit.clone()) {
            ...
            let delete_closure = Closure::wrap(Box::new(move |_event: web_sys::MouseEvent| {
                ...
                match window.confirm_with_message("Are you sure you want to delete this access level?") {
                    Ok(confirmed) => {
                        if confirmed {
                            if let Err(e) = AccessLevelView::handle_delete_item(view_rc_clone_for_delete.clone(), item_id_for_delete) {
                                console_log!("Error initiating delete item: {:?}", e);
                            }
                        } else {
                            console_log!("AccessLevelView: Delete cancelled by user.");
                        }
                    }
                    Err(e) => {
        ...
        console_log!("AccessLevelView: Records rendered.");
        Ok(())
    }
    ...
    pub fn populate_add_form(view_rc: Rc<RefCell<Self>>) -> Result<(), JsValue> {
        ...
        *view.current_op.borrow_mut() = Some(UserAgeGroupOperation::Adding);
        ...
        name_input_el.set_placeholder("Enter access level name");
        ...
        let save_closure = Closure::wrap(Box::new(move |_event: web_sys::MouseEvent| {
            if let Err(e) = AccessLevelView::handle_save_action(view_clone_for_save.clone()) {
        ...
        let cancel_closure = Closure::wrap(Box::new(move |_event: web_sys::MouseEvent| {
            if let Err(e) = AccessLevelView::hide_edit_form(view_clone_for_cancel.clone()) {
        ...
        console_log!("AccessLevelView: Add form populated.");
        Ok(())
    }
    ...
    pub fn populate_edit_form(view_rc: Rc<RefCell<Self>>, item: TableData) -> Result<(), JsValue> {
        ...
        *view.current_op.borrow_mut() = Some(UserAgeGroupOperation::Editing(item.id));
        ...
        console_log!("AccessLevelView: Edit form populated for item ID {}.", item.id);
        Ok(())
    }
    ...
    pub fn handle_save_action(view_rc: Rc<RefCell<Self>>) -> Result<(), JsValue> {
        ...
        let name_value = match &view_mut.name_input {
            ...
            None => {
                console_log!("AccessLevelView: Name input not found.");
        ...
        let description_value = match &view_mut.description_input {
            Some(input) => input.value(),
            None => {
                console_log!("AccessLevelView: Description input not found.");
                // Description can be optional, so don't necessarily error out, or handle as per requirements
                String::new() // Default to empty if not found, or handle error
            }
        };

        if name_value.trim().is_empty() {
            console_log!("AccessLevelView: Name cannot be empty.");
            view_mut.state_div.set_inner_text("Error: Name cannot be empty.");
            return Ok(()); // Keep form open
        }
        // Description can be empty, so no validation for it here unless specified.

        match operation {
            Some(UserAgeGroupOperation::Adding) => {
                console_log!("AccessLevelView: Preparing POST for name: {}, description: {}", name_value, description_value);
                let payload = AccessLevelCreate { name: name_value, description: description_value };
                    ...
                    match http_client.post(API_URL).json(&payload).send().await {
                        Ok(response) => {
                            if response.status().is_success() || response.status().as_u16() == 201 {
                                console_log!("AccessLevelView: Successfully added access level. Status: {}", response.status());
                                AccessLevelView::fetch_items(view_rc_clone_for_async);
                            } else {
                                let status_code = response.status();
                                let err_text = response.text().await.unwrap_or_else(|_| "Failed to get error text".to_string());
                                console_log!("AccessLevelView: Failed to add access level. Status: {}. Error: {}", status_code, err_text);
                                // view_rc_clone_for_async.borrow_mut().state_div.set_inner_text(&format!("Error adding: {} - {}", status_code, err_text));
                            }
                        }
                        Err(e) => {
                            console_log!("AccessLevelView: Network error during POST: {:?}", e);
            ...
            Some(UserAgeGroupOperation::Editing(item_id)) => {
                console_log!("AccessLevelView: Preparing PUT for item ID {}: name: {}, description: {}", item_id, name_value, description_value);
                let payload = AccessLevelUpdate { name: name_value, description: description_value };
                ...
                spawn_local(async move {
                    ...
                    match http_client.put(&request_url).json(&payload).send().await {
                        Ok(response) => {
                            if response.status().is_success() {
                                console_log!("AccessLevelView: Successfully updated item_id {}. Status: {}", item_id, response.status());
                                AccessLevelView::fetch_items(view_rc_clone_for_async);
                            } else {
                                let status_code = response.status();
                                let err_text = response.text().await.unwrap_or_else(|_| "Failed to get error text".to_string());
                                console_log!("AccessLevelView: Failed to update item_id {}. Status: {}. Error: {}", item_id, status_code, err_text);
                                // view_rc_clone_for_async.borrow_mut().state_div.set_inner_text(&format!("Error updating: {} - {}", status_code, err_text));
                            }
                        }
                        Err(e) => {
                            console_log!("AccessLevelView: Network error during PUT for item_id {}: {:?}", item_id, e);
            ...
            None => {
                 console_log!("AccessLevelView: No operation active in handle_save_action.");
            }
        }
        AccessLevelView::hide_edit_form(view_rc.clone())?; // Call synchronously
        Ok(())
    }
    ...
    pub fn hide_edit_form(view_rc: Rc<RefCell<Self>>) -> Result<(), JsValue> {
        ...
        view.description_input = None; // Ensure description_input is also cleared
        *view.current_op.borrow_mut() = None;
        console_log!("AccessLevelView: Edit form hidden.");
        Ok(())
    }

    pub fn handle_delete_item(view_rc: Rc<RefCell<Self>>, item_id: i32) -> Result<(), JsValue> {
        console_log!("AccessLevelView: Attempting to delete item_id {}.", item_id);
            ...
            match http_client.delete(&request_url).send().await {
                Ok(response) => {
                    if response.status().is_success() || response.status().as_u16() == 204 { // 204 No Content
                        console_log!("AccessLevelView: Successfully deleted item_id {}. Status: {}", item_id, response.status());
                        AccessLevelView::fetch_items(view_rc_clone_for_async);
                    } else {
                        let status = response.status();
                        let err_text = response.text().await.unwrap_or_else(|_| "Failed to get error text".to_string());
                        console_log!(
                            "AccessLevelView: Failed to delete item_id {}. Status: {}. Error: {}",
                            item_id,
                            status,
                            err_text
                        );
                ...
                Err(e) => {
                    console_log!("AccessLevelView: Network error during DELETE for item_id {}: {:?}", item_id, e);
