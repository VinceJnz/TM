...
func (h *Handler) GetAllQry() ([]models.GroupResource, error) {
	...
	err := h.appConf.Db.Select(&records, `SELECT id, group_id, recource_id, access_level_id, access_type_id, admin_flag FROM st_group_resource`)
	if err != nil {
		return nil, err
	}
	return records, nil
...
func (h *Handler) GetQry(id int) (models.GroupResource, error) {
	...
	err := h.appConf.Db.Get(&record, "SELECT id, group_id, recource_id, access_level_id, access_type_id, admin_flag FROM st_group_resource WHERE id = $1", id)
	if err != nil {
...
func (h *Handler) CreateQry(record models.GroupResource) error {
	err := h.appConf.Db.QueryRow(
		"INSERT INTO st_group_resource (group_id, recource_id, access_level_id, access_type_id, admin_flag) VALUES ($1, $2, $3, $4, $5) RETURNING id",
...
func (h *Handler) UpdateQry(record models.GroupResource) error {
	_, err := h.appConf.Db.Exec("UPDATE st_group_resource SET group_id = $1, recource_id = $2, access_level_id = $3, access_type_id = $4, admin_flag = $5 WHERE id = $6",
