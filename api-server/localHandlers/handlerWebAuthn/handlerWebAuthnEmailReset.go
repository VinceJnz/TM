...
func (h *Handler) EmailResetRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	tempSessionToken, err := dbAuthTemplate.CreateNamedToken(debugTag+"Handler.BeginEmailResetHandler()4 ", h.appConf.Db, true, user.ID, h.appConf.Settings.Host, WebAuthnSessionTokenName, time.Now().Add(3*time.Minute))
	if err != nil {
...
func (h *Handler) EmailResetListHandler(w http.ResponseWriter, r *http.Request) {
	...
	tempSessionToken, err := dbAuthTemplate.CreateNamedToken(debugTag+"Handler.BeginEmailResetHandler()4 ", h.appConf.Db, false, user.ID, h.appConf.Settings.Host, WebAuthnSessionTokenName, time.Now().Add(3*time.Minute))
	if err != nil {
		http.Error(w, "Failed to create session token", http.StatusInternalServerError)
		log.Printf("%v %v %v %v %v %v %v", debugTag+"Handler.BeginRegistration()4: Failed to create session token", "err =", err, "WebAuthnSessionTokenName =", WebAuthnSessionTokenName, "host =", h.appConf.Settings.Host)
		return
