...
func (h *Handler) BeginRegistration(w http.ResponseWriter, r *http.Request) {
	...
	tempRegistrationToken, err := dbAuthTemplate.CreateNamedToken(debugTag+"Handler.BeginRegistration()7 ", h.appConf.Db, false, user.ID, h.appConf.Settings.Host, WebAuthnSessionTokenName, time.Now().Add(3*time.Minute))
	if err != nil {
		http.Error(w, "Failed to create session token", http.StatusInternalServerError)
