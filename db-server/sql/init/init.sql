...
CREATE TABLE IF NOT EXISTS st_users (
    ...
    provider VARCHAR(30) DEFAULT NULL, -- OAuth provider name (e.g., "google", "facebook")
    provider_id VARCHAR(255) DEFAULT NULL, -- OAuth provider user ID
    created TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    --FOREIGN KEY (user_age_group_id) REFERENCES et_user_age_group(id)
...
CREATE TABLE st_webauthn_credentials (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL,
    credential_id BYTEA NOT NULL, -- Stores the credential ID generated by the WebAuthn API
    credential_data JSONB NOT NULL,
    last_used TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- Tracks the last time the credential was used
