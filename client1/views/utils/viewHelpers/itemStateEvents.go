...
func SetItemState(events *eventProcessor.EventProcessor, state *ItemState, newState ItemState, debugTag string) {
	if events != nil {
		events.ProcessEvent(eventProcessor.Event{Type: "updateStatus", DebugTag: debugTag, Data: newState})
	}
	if state != nil {
		*state = newState
	}
...
func SetItemStateFromLocal[T ~int](events *eventProcessor.EventProcessor, state *T, newState T, debugTag string) {
	if events != nil {
		events.ProcessEvent(eventProcessor.Event{Type: "updateStatus", DebugTag: debugTag, Data: ItemState(newState)})
	}
	if state != nil {
		*state = newState
	}
